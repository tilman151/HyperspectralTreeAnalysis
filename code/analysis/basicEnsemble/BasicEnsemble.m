classdef BasicEnsemble < Classifier
    ...
    methods
        function obj = BasicEnsemble(baseClassifier, numClassifier, trainingInstanceProportions)
        ...
        function obj = trainOn(obj, trainFeatures, trainLabels)
            ...
            randomizedIndices = cellfun(@createRandPerm, repmat({x*y}, 1, length(obj.trainingInstanceProportions)), obj.trainingInstanceProportions, 'UniformOutput', 0)';
            % make features 2D
            reshapedTrainFeatures = repmat({reshape(trainFeatures, x*y, spectralBands)}, length(obj.trainingInstanceProportions), 1);
            reshapedTrainLabels = repmat({reshape(trainLabels, x*y, 1)}, length(obj.trainingInstanceProportions), 1);
